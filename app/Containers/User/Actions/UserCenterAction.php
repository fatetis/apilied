...
class UserCenterAction extends Action
{
    public function run()
    {
        ...
        $order_result = Apiato::call('Order@GetOrderBaseByUserIdAndStatusTask', [[
            OrderBase::ORDER_STATUS_WAIT_PAY,
            OrderBase::ORDER_STATUS_WAIT_DELIVERY,
            OrderBase::ORDER_STATUS_WAIT_PART_DELIVERY,
            OrderBase::ORDER_STATUS_WAIT_TAKE,
            OrderBase::ORDER_STATUS_WAIT_APPRAISE,
            OrderBase::ORDER_STATUS_REFUNDING,
        ], $user_info['id']]);
        collect($order_result)->map(function($value) use(&$data) {
            if($value->order_status == OrderBase::ORDER_STATUS_WAIT_PAY) {
                !isset($data['order']['wait_pay']) && $data['order']['wait_pay'] = 0;
                $data['order']['wait_pay']++;
            } elseif (in_array($value->order_status, [OrderBase::ORDER_STATUS_WAIT_DELIVERY, OrderBase::ORDER_STATUS_WAIT_PART_DELIVERY])){
                !isset($data['order']['wait_delivery']) && $data['order']['wait_delivery'] = 0;
                $data['order']['wait_delivery']++;
            } elseif ($value->order_status == OrderBase::ORDER_STATUS_WAIT_TAKE) {
                !isset($data['order']['wait_take']) && $data['order']['wait_take'] = 0;
                $data['order']['wait_take']++;
            } elseif ($value->order_status == OrderBase::ORDER_STATUS_WAIT_APPRAISE) {
                !isset($data['order']['wait_appraise']) && $data['order']['wait_appraise'] = 0;
                $data['order']['wait_appraise']++;
            } elseif ($value->order_status == OrderBase::ORDER_STATUS_REFUNDING) {
        ...
        $ename = ['user_center'];
        $data['adv'] = Apiato::call('Index@GetAdvByEnameTask', [['ename' => $ename]]);
        return $data;
